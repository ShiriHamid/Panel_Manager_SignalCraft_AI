...
// Get blocks for a table in current session — returns array of block objects
const getTableBlocks = (blockedForDate, tableId, sess, sessName) => {
  return blockedForDate.filter(b => {
    ...
    if (b.start_time && b.end_time) {
      // Has specific times — check overlap with session
      return toMin(b.start_time) < sess.end && toMin(b.end_time) > sess.start;
    }
    // Session-based block
...
export default function App() {
  ...
  const addBlock = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        showToast("🚫 Table blocked");
        await fetchBlocked();
        setBlockForm({ table_id:"T01", date:toDateStr(0), session:"All", start_time:"", end_time:"", reason:"" });
      } else {
        showToast("Failed to block table", "err");
      }
    } catch (e) {
      showToast("Failed to block table", "err");
    } finally {
  ...
  const removeBlock = async (id) => {
    try {
      ...
      showToast("Failed to remove block", "err");
    }
  };
  ...
  // For a block bar: clamp to session boundaries
  const blockLeftPct = (b) => {
    const start = b.start_time ? Math.max(toMin(b.start_time), sess.start) : sess.start;
    return Math.max(0, (start - sess.start) / sessW * 100);
  };
  const blockWidthPct = (b) => {
    const start = b.start_time ? Math.max(toMin(b.start_time), sess.start) : sess.start;
    const end   = b.end_time   ? Math.min(toMin(b.end_time),   sess.end)   : sess.end;
    return Math.min(100, (end - start) / sessW * 100);
  };
      ...
      <div style={{width:200,background:"#0f0f1c",borderRight:"1px solid #1a1a30",display:"flex",flexDirection:"column",flexShrink:0}}>
        ...
        <nav style={{flex:1,padding:"12px 8px"}}>
          {[
            ...
            {id:"list",icon:"📋",label:"Reservations"},
            {id:"floor",icon:"🪑",label:"Floor Map"},
            {id:"blocked",icon:"🚫",label:"Block Tables"},
          ].map(item=>(
              ...
              <div style={{padding:"16px 18px",overflow:"auto"}}>
                    ...
                    return (
                      <div key={t.id} style={{display:"flex",alignItems:"center",marginBottom:6,height:36}}>
                        ...
                        <div style={{flex:1,position:"relative",height:30,background:isFullyBlocked?"#2b0a0a":"#0f0f1c",borderRadius:6,overflow:"hidden",border:isFullyBlocked?"1px solid #991b1b":"none"}}>
                          ...
                          {!isFullyBlocked && tRes.map(r=>{
                            ...
                            return (
                              <div key={r.id} onClick={()=>setDetail(r)}
                                ...
                                style={{position:"absolute",top:2,height:"calc(100% - 4px)",left:`${lp}%`,width:`${wp}%`,background:sc.bg,border:`1px solid ${sc.border}`,borderRadius:5,cursor:"pointer",display:"flex",alignItems:"center",paddingLeft:6,overflow:"hidden",minWidth:4,zIndex:2}}>
                          ...
                          {/* Block bars — shown as red bars with exact time range */}
                          {tBlocks.map(b=>{
                            if (isFullyBlocked) return null; // handled by full red row
                            const lp = blockLeftPct(b);
                            const wp = blockWidthPct(b);
                            if(wp<=0) return null;
                            return (
                              <div key={b.id}
                                title={`Blocked${b.reason ? ` — ${b.reason}` : ""}${b.start_time ? ` (${b.start_time}–${b.end_time})` : ""}`}
                                style={{position:"absolute",top:2,height:"calc(100% - 4px)",left:`${lp}%`,width:`${wp}%`,background:"#2b0a0a",border:"1px solid #991b1b",borderRadius:5,display:"flex",alignItems:"center",paddingLeft:6,overflow:"hidden",minWidth:4,zIndex:3}}>
                                <span style={{fontSize:10,fontWeight:600,color:"#ef4444",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
                                  {wp>8 ? `🚫${b.reason ? ` ${b.reason}` : ""}` : ""}
                                </span>
                              </div>
                            );
                          })}
                          ...
                          {isFullyBlocked && (
                            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",paddingLeft:8}}>
                              <span style={{fontSize:10,color:"#ef4444",fontWeight:600}}>
                                Blocked{tBlocks[0]?.reason ? ` — ${tBlocks[0].reason}` : ""}
                              </span>
                            </div>
                          )}

                          {/* Free label */}
                          {!isFullyBlocked && tRes.length===0 && tBlocks.length===0 && (
                            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",paddingLeft:8}}>
                ...
                <div style={{display:"flex",gap:16,marginTop:12,paddingTop:12,borderTop:"1px solid #1a1a30"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:6,fontSize:11}}>
                    <div style={{width:12,height:12,borderRadius:3,background:"#2b0a0a",border:"1px solid #991b1b"}}/>
            ...
            <div style={{background:"#13131f",border:"1px solid #1a1a30",borderRadius:12,overflow:"hidden",flex:1}}>
              <div style={{padding:"12px 18px",borderBottom:"1px solid #1a1a30",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                ...
                <div style={{display:"flex",gap:10}}>
                  {[["#22c55e","Free"],["#f59e0b","Pending"],["#ef4444","Occupied"],["#991b1b","Blocked"]].map(([c,l])=>(
                    ...
                    return (
                      <div key={t.id} onClick={()=>activeRes&&!isBlocked&&setDetail(activeRes)}
                        ...
                        <div style={{fontSize:20,marginBottom:4}}>
                          {isBlocked?"🚫":t.capacity<=2?"🪑":t.capacity<=4?"🍽️":t.capacity<=6?"👥":"🏛️"}
                        </div>
                        ...
                        {isBlocked && (
                          <div style={{fontSize:9,color:"#ef4444",marginTop:4,fontWeight:600}}>
                            {blockInfo?.reason || "Blocked"}<br/>
                            {blockInfo?.session || ""}{blockInfo?.start_time ? ` ${blockInfo.start_time}–${blockInfo.end_time}` : ""}
                          </div>
                        )}
                        {!isBlocked && activeRes && (
            ...
            <div style={{display:"flex",flexDirection:"column",gap:16,flex:1}}>
              <div style={{background:"#13131f",border:"1px solid #1a1a30",borderRadius:12,padding:20}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:12}}>
                  <div>
                    <div style={{fontSize:11,color:"#475569",marginBottom:6}}>Table</div>
                    <select value={blockForm.table_id} onChange={e=>setBlockForm(p=>({...p,table_id:e.target.value}))}
                      style={{width:"100%",background:"#0f0f1c",border:"1px solid #1a1a30",borderRadius:8,padding:"8px 10px",color:"#e2e8f0",fontSize:13}}>
                      {TABLES.map(t=><option key={t.id} value={t.id}>{t.id} — {t.capacity}p {t.location}</option>)}
                    </select>
                  </div>
                  <div>
                    <div style={{fontSize:11,color:"#475569",marginBottom:6}}>Date</div>
                    <input type="date" value={blockForm.date} onChange={e=>setBlockForm(p=>({...p,date:e.target.value}))}
                      style={{width:"100%",background:"#0f0f1c",border:"1px solid #1a1a30",borderRadius:8,padding:"8px 10px",color:"#e2e8f0",fontSize:13,boxSizing:"border-box"}}/>
                  </div>
                  <div>
                    <div style={{fontSize:11,color:"#475569",marginBottom:6}}>Session</div>
                    <select value={blockForm.session} onChange={e=>setBlockForm(p=>({...p,session:e.target.value}))}
                      style={{width:"100%",background:"#0f0f1c",border:"1px solid #1a1a30",borderRadius:8,padding:"8px 10px",color:"#e2e8f0",fontSize:13}}>
                      {SESSION_OPTIONS.map(s=><option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                  <div>
                    <div style={{fontSize:11,color:"#475569",marginBottom:6}}>Start Time <span style={{color:"#374151"}}>(optional)</span></div>
                    <input type="time" value={blockForm.start_time} onChange={e=>setBlockForm(p=>({...p,start_time:e.target.value}))}
                      style={{width:"100%",background:"#0f0f1c",border:"1px solid #1a1a30",borderRadius:8,padding:"8px 10px",color:"#e2e8f0",fontSize:13,boxSizing:"border-box"}}/>
                  </div>
                  <div>
                    <div style={{fontSize:11,color:"#475569",marginBottom:6}}>End Time <span style={{color:"#374151"}}>(optional)</span></div>
                    <input type="time" value={blockForm.end_time} onChange={e=>setBlockForm(p=>({...p,end_time:e.target.value}))}
                      style={{width:"100%",background:"#0f0f1c",border:"1px solid #1a1a30",borderRadius:8,padding:"8px 10px",color:"#e2e8f0",fontSize:13,boxSizing:"border-box"}}/>
                  </div>
                  <div>
                    <div style={{fontSize:11,color:"#475569",marginBottom:6}}>Reason <span style={{color:"#374151"}}>(optional)</span></div>
                    <input type="text" placeholder="e.g. Maintenance" value={blockForm.reason} onChange={e=>setBlockForm(p=>({...p,reason:e.target.value}))}
                      style={{width:"100%",background:"#0f0f1c",border:"1px solid #1a1a30",borderRadius:8,padding:"8px 10px",color:"#e2e8f0",fontSize:13,boxSizing:"border-box"}}/>
                  </div>
                </div>
              ...
              <div style={{background:"#13131f",border:"1px solid #1a1a30",borderRadius:12,overflow:"hidden",flex:1}}>
                <div style={{padding:"12px 18px",borderBottom:"1px solid #1a1a30",fontWeight:700,fontSize:14}}>
                  Active Blocks ({blocked.length})
                </div>
                <div style={{overflow:"auto"}}>
                  {blocked.length === 0 ? (
                    <div style={{padding:32,textAlign:"center",color:"#374151",fontSize:14}}>No blocked tables</div>
                  ) : (
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
                      <thead>
                        <tr style={{background:"#0f0f1c"}}>
                          {["Table","Date","Session","Time","Reason",""].map(h=>(
                            <th key={h} style={{padding:"9px 14px",textAlign:"left",fontSize:11,color:"#475569",fontWeight:700}}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {blocked.map((b,i)=>(
                          <tr key={b.id} style={{borderTop:"1px solid #1a1a30",background:i%2===0?"transparent":"#0f0f1c08"}}>
                            <td style={{padding:"11px 14px"}}>
                              <span style={{background:"#2b0a0a",border:"1px solid #991b1b",borderRadius:6,padding:"2px 8px",fontSize:12,fontWeight:700,color:"#ef4444"}}>{b.table_id}</span>
                            </td>
                            ...
                            <td style={{padding:"11px 14px"}}>
                              <button onClick={()=>removeBlock(b.id)}
                                style={{background:"#2b0a0a",color:"#ef4444",border:"1px solid #991b1b",borderRadius:6,padding:"4px 10px",fontSize:11,fontWeight:700,cursor:"pointer"}}>
                                Remove
                              </button>
                            </td>
                          </tr>
