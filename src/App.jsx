...
// ✅ Smart block check: respects start/end time if present, otherwise uses session
const isTableBlocked = (blockedForDate, tableId, sess, sessionName) => {
  return blockedForDate.some(b => {
    if (b.table_id !== tableId) return false;
    // If specific times are set, use overlap logic
    if (b.start_time && b.end_time) {
      return toMin(b.start_time) < sess.end && toMin(b.end_time) > sess.start;
    }
    // Otherwise use session matching
    return b.session === "All" || b.session === sessionName;
  });
};

const getBlockInfo = (blockedForDate, tableId, sess, sessionName) => {
  return blockedForDate.find(b => {
    if (b.table_id !== tableId) return false;
    if (b.start_time && b.end_time) {
      return toMin(b.start_time) < sess.end && toMin(b.end_time) > sess.start;
    }
              ...
              <div style={{padding:"16px 18px",overflow:"auto"}}>
                ...
                <div style={{position:"relative"}}>
                  ...
                  {TABLES.map(t=>{
                    ...
                    const isBlocked = isTableBlocked(blockedForDate, t.id, sess, sessName);
                    const blockInfo = getBlockInfo(blockedForDate, t.id, sess, sessName);
                    return (
                      <div key={t.id} style={{display:"flex",alignItems:"center",marginBottom:6,height:36}}>
                        <div style={{width:90,flexShrink:0,display:"flex",alignItems:"center",gap:6}}>
                          <span style={{fontSize:12,fontWeight:700,color:isBlocked?"#ef4444":"#a78bfa"}}>{t.id}</span>
                          <span style={{fontSize:10,color:"#374151"}}>{t.capacity}p</span>
                          {isBlocked && <span style={{fontSize:9,color:"#ef4444"}}>🚫</span>}
                        </div>
                        <div style={{flex:1,position:"relative",height:30,background:isBlocked?"#2b0a0a":"#0f0f1c",borderRadius:6,overflow:"hidden",border:isBlocked?"1px solid #991b1b":"none"}}>
                          {isBlocked ? (
                            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",paddingLeft:8}}>
                              <span style={{fontSize:10,color:"#ef4444",fontWeight:600}}>
                                Blocked{blockInfo?.reason ? ` — ${blockInfo.reason}` : ""}
                                {blockInfo?.start_time && blockInfo?.end_time ? ` (${blockInfo.start_time}–${blockInfo.end_time})` : ""}
                              </span>
                            </div>
                          ) : (
                            <>
